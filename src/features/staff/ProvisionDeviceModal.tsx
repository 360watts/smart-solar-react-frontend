...
/**
 * Generates a single-use claim token for a device MAC, to be entered/scanned
 * into the device during its setup flow — the device then sends it back as
 * claimNonce in POST /devices/provision. Without this, /devices/provision
 * would accept any hwId with no proof the caller actually owns that hardware.
 */
export const ProvisionDeviceModal: React.FC<ProvisionDeviceModalProps> = ({ open, onClose }) => {
  ...
  const generate = async () => {
    ...
    if (!trimmed) {
      setError('Enter the device MAC address first');
      return;
    }
    setLoading(true);
    setError(null);
    try {
      const result = await apiService.createDeviceClaim(trimmed);
      setClaim(result);
    } catch (err) {
  ...
  const S: Record<string, React.CSSProperties> = {
    ...
    input: {
      ...
      fontFamily: 'Fira Code, JetBrains Mono, monospace', fontSize: '0.85rem',
    ...
    <div style={S.overlay} onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div style={S.panel} role="dialog" aria-modal="true" aria-labelledby="provision-modal-title">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: '0.75rem', marginTop: 2, color: 'var(--muted-foreground)' }}>
                Generate a claim token for a technician to enter on-site
              </div>
            </div>
          </div>
        ...
        <div style={S.body}>
          {!claim ? (
            <>
              <label style={{ fontSize: '0.75rem', marginBottom: 6, display: 'block', color: 'var(--muted-foreground)' }}>
                Device MAC address
              </label>
              <input
                autoFocus
              ...
              <button style={S.generateBtn} onClick={generate} disabled={loading}>
                {loading ? 'Generating…' : 'Generate Claim Token'}
              </button>
            </>
          ) : (
            <div style={{ textAlign: 'center' }}>
              ...
              <div style={{ fontSize: '0.775rem', color: 'var(--muted-foreground)', marginBottom: 4 }}>
                Scan or enter this token on the device for MAC <strong>{claim.hwId}</strong> during setup.
              </div>
